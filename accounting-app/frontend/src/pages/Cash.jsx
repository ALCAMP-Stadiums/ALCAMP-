...
export default function Cash() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (to) params.to = to;
      const res = await axios.get('/api/cash/transactions', { params });
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      await axios.post('/api/cash/transactions', form);
      setShowModal(false);
  ...
  const printHeaders = ['#', 'التاريخ', 'البيان', 'المرجع', 'وارد', 'صادر', 'الرصيد', 'ملاحظات'];
  ...
  return (
    <div className="space-y-6" dir="rtl">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">الخزينة</h1>
          <p className="text-gray-500 text-sm">إدارة المعاملات النقدية</p>
        </div>
      ...
      <div className="card flex items-center gap-4 flex-wrap">
        ...
        <button onClick={fetchData} className="btn-primary text-sm py-2">
          بحث
        </button>
        <button onClick={() => { setFrom(''); setTo(''); setTimeout(fetchData, 0); }} className="btn-secondary text-sm py-2">
          إعادة تعيين
        </button>
      </div>
      ...
      <div className="card overflow-hidden p-0">
        <div className="p-4 border-b border-gray-100">
          <h2 className="font-bold text-gray-700">كشف الخزينة ({transactions.length} معاملة)</h2>
        </div>
        {loading ? (
              ...
              <tbody>
                {transactions.length === 0 ? (
                  ...
                  <tr key={tx.id}>
                    ...
                    {isAccountant() && (
                      <td>
                        <button onClick={() => handleDelete(tx.id)} className="btn-danger text-xs py-1 px-2">
                          حذف
                        </button>
                      </td>
                    )}
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="إضافة حركة خزينة">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">مبلغ وارد</label>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">مبلغ صادر</label>
          ...
          <div>
            ...
            <textarea value={form.notes} onChange={(e) => setForm({...form, notes: e.target.value})} className="input-field" rows="2" placeholder="ملاحظات إضافية" />
          </div>
